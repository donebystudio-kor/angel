...
#### 네 번째 재발 — 지시 자체가 틀이 되는 경우 (트랙 3)

트랙 3에서 규칙 E가 한 배치 안에서 세 곳 동시 발생했다.
- "999 (completion, amplified)" 오프닝 절 4/4
- spiritual "Worth sitting with:" 오프너 4/4
- Q2의 999X 서술 4/4 동일 문형

세 번째가 특히 중요하다. "Q2에서 999X를 느슨하게 서술하라"는 지시를
따르는 과정에서 4개가 **같은 방식으로 느슨해졌다.**

**지시가 구체적일수록 그 지시가 새 틀이 된다.**
"~하게 쓰라"는 지시를 받으면 그 방식 하나로 수렴한다.

**대응**: 지시를 받은 항목일수록 초안 직후 먼저 검사하라.
"시킨 대로 했으니 괜찮다"고 넘기지 마라.
...
### 규칙 J — 묶음 검사의 한계, 통합 검사는 항상 필수

트랙 4(999X 7개, 4개+3개 두 묶음)에서 Q2 오프닝 "reverse order/
pairing"이 7개 중 5개에 수렴했다. 묶음별 검사에서는 잡히지 않았다 —
각 묶음에서는 2~3개씩이라 상한(구조당 6개 등)에 안 걸렸기 때문이다.
전체를 합친 통합 검사에서야 5/7이라는 압도적 비율로 드러났다.

**묶음이 몇 개든, 묶음별 검사만으로는 부족하다.** 수렴 비율은 전체
모집단에서만 보인다 — 묶음 하나에 2~3개씩 나눠 들어가 있으면 각
묶음은 상한 이내로 보여도 합치면 절반을 넘을 수 있다.

**대응**:
1. 묶음별 검사(재작성 범위를 줄이기 위한 것) — 계속 유지
2. **전체 통합 검사(필수)** — 묶음 수와 무관하게 항상 마지막에 실행
3. 통합 검사에서 한 패턴이 전체의 40%를 넘으면 재작성

특히 Q2처럼 기능이 고정된 필드(항상 "대응 계열과 비교"라는 같은
역할을 함)는 묶음별로는 다양해 보여도 통합 검사에서만 수렴이
드러난다.

### 규칙 K — 자기 페이지 내부 필드 간 중복 (배치 8에서 최초 발견)

1113·1116의 manifestation이 같은 페이지 action 필드를 거의 그대로
재진술하고 있었다.

원인: X111 트랙에서 action을 meaning 축에 맞춰 재작성했고,
manifestation도 같은 축으로 쓰니 둘이 만났다. **축이 같으면 필드가
달라도 내용이 겹친다.**

**대조 범위 확정 (배치12부터 고정 — 이 범위 외로 좁히지 말 것)**:
manifestation 작성 시 같은 페이지의 아래 전체와 대조한다.
- meaning **전체** (마지막 문장만이 아니라 전체 본문)
- spiritual
- action (가장 위험 — 둘 다 "무엇을 하라"는 지시문이다)
- FAQ **전 문항** (질문과 답변 모두)

배치11에서 대조 범위를 action/spiritual + meaning 마지막 문장으로
좁게 잡았다가, 배치11 통합 검사에서 범위를 meaning 전체·FAQ까지
넓히자 그때는 안 보였던 위반 10건이 새로 드러났다(0404는 FAQ Q1
답변의 문장 구조를 통째로 가져온 수준). **필드 일부만 보면 위반을
놓친다 — 항상 네 범위 전체를 본다.**

특히 meaning·action이 이미 재작성된 페이지(X111 트랙 31개, 한국어
트랙 전체)는 축이 공유되므로 반드시 확인한다.

### 규칙 K를 작성 전 제약으로 전환 (배치13부터)

배치11(10건) → 배치12(18건)로 규칙K 위반이 계속 늘었다. mixed
항목이 배치마다 다수(13개) 포함되기 시작하면서, mixed의 meaning
필드가 이미 다른 형제 숫자와의 비교를 상세히 서술해 놓은 경우가
많아 그 서술을 그대로 재사용하려는 유혹이 특히 컸기 때문이다.

A형 골격 grep을 "검증 단계"에서 "작성 절차"로 옮겼더니 위반이
3건→0건이 된 전례(위 "절차 개선 실측 기록" 참조)를 규칙K에도
그대로 적용한다.

**작성 전, 각 페이지마다 (필수 절차)**:
1. 그 페이지의 meaning 전체·spiritual·action·FAQ 전 문항에서 3-gram을
   전부 추출한다
2. 숫자 표준 어휘(1=시작, 5=변화 등, 규칙 G 참조)는 제외한다
3. 나머지를 **그 페이지 전용 금지 목록**으로 만든다
4. manifestation을 이 목록을 피해서 쓴다
5. 작성 후 규칙K 검사(전체 3-gram 대조)는 그대로 유지한다 — 이중
   방어

**배치13 실측**: 이 절차를 적용한 1차 초안에서 규칙K 위반이 **4건**
으로 나왔다(배치12는 검증 단계에서만 걸러 18건이었음). 작성 전
제약이 실제로 위반 발생률을 크게 낮춘다는 게 확인됨 — 계속 유지.
